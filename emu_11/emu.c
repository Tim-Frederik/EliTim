...
// The following check is performed in theory.c
// Check the inputs to make sure we're interpolating.
    ...
    for(i=0; i<nsim; i++) {
        ...
        pdelta[i] = gsl_spline_eval(lininterp_z, xstar[5], accel);
        gsl_interp_accel_reset(accel);
    }
    ...
    switch(outtype[0]) {
        ...
        case 1:
            ...
            for(j=0; j<nsim; j++) {
                pdelta[j] = pdelta[j] + 1.5*log10(ksim[j]);
                pdelta[j] = pow(10.0,pdelta[j]);
            }
            break;
        case 2:
            // Transform to P(k)
            for(j=0; j<nsim; j++) {
                pdelta[j] = pdelta[j] - 1.5*log10(ksim[j]);
                pdelta[j] = pow(10.0,pdelta[j])*2.0*M_PI*M_PI;
            }
            break;
    }
