...
void make_cov_power_tomo(char *OUT_FILE, char *PARA_FILE,double *ell,double *dell,int Ncl)
{
  
}


void make_cov_tpcf_no_tomo(char *OUT_FILE, char *PARA_FILE,double *vt,double *dvt,int Ntpcf)
{
  
}

void make_cov_tpcf_tomo(char *OUT_FILE, char *PARA_FILE,double *vt,double *dvt,int Ntpcf)
{
  
}


void make_cov_cosebis_no_tomo(char *OUT_FILE, char *PARA_FILE,double vtmax,double vtmin,int Ncosebis)
{
  
}

void make_cov_cosebis_tomo(char *OUT_FILE, char *PARA_FILE,double vtmax,double vtmin,int Ncosebis)
{
...
void write_parameters(char *PARA_FILE, char *OUT_FILE,int N_para,int START,int END,int Ncl,int Ntpcf,int Ncosebis, double lmax, double lmin, double vtmax, double vtmin, int set_redshift)
{
  ...
  printf("\n");
  if (Ncl!=0){ 
    printf("# Computing power spectrum covariance %d\n");
    printf("# bins CL: %d\n",Ncl);
    printf("lmax: %le\n",lmax);
    printf("lmin: %le\n",lmin);
  }
  if (Ntpcf!=0){  
    printf("# Computing correlation function covariance %d\n");
    printf("# bins tpcf: %d\n",Ntpcf);
    printf("vtmax: %le\n",vtmax);
    printf("vtmin: %le\n",vtmin);
  }
  ...
  if (Ncosebis!=0){  
    printf("# Computing Cosebis covariance %d\n");
    printf("# modes Cosebis: %d\n",Ncosebis);
    printf("vtmax: %le\n",vtmax);
    printf("vtmin: %le\n",vtmin);
  }
